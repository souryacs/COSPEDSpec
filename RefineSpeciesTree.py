...
def ResolveMultifurcation(Gene_TreeList, Curr_tree, clust_species_list, no_of_input_clusters, Output_Text_File):
  ...
  for i in range(no_of_clust - 1):
	...
	#Level_Score_Mat[j][i] = Level_Score_Mat[i][j] = TaxaPair_Reln_Dict[key1]._GetXLSumGeneTrees()
	Level_Score_Mat[j][i] = Level_Score_Mat[i][j] = TaxaPair_Reln_Dict[key1]._GetNormalizedXLSumGeneTrees()
      elif key2 in TaxaPair_Reln_Dict:
	#Level_Score_Mat[j][i] = Level_Score_Mat[i][j] = TaxaPair_Reln_Dict[key2]._GetXLSumGeneTrees()
	Level_Score_Mat[j][i] = Level_Score_Mat[i][j] = TaxaPair_Reln_Dict[key2]._GetNormalizedXLSumGeneTrees()
      else:
	Level_Score_Mat[j][i] = Level_Score_Mat[i][j] = 0
  # end add - sourya
  #---------------------------------------
  # comment - sourya
  ## add - sourya
  #for i in range(no_of_clust - 1):
	...
	#for k2 in range(len(clust_species_list[j])):	  
      ...
      #Level_Score_Mat[j][i] = Level_Score_Mat[i][j] = (sum(curr_taxa_pair_list) * 1.0) / len(curr_taxa_pair_list)
  ## end add - sourya
  # end comment - sourya
  #---------------------------------------
  ...
  while(no_of_clust > 2): 
    ...
    # present in the matrix Norm_Level_Score_Mat
	...
	if (Norm_Level_Score_Mat[i][j] < min_val):
	  ...
	  min_idx_i = i
	  min_idx_j = j
	elif (Norm_Level_Score_Mat[i][j] == min_val):
    ...
    for k in range(no_of_clust):
      ...
      Level_Score_Mat[k][no_of_clust] = max(Level_Score_Mat[k][min_idx_i], Level_Score_Mat[k][min_idx_j], Level_Score_Mat[min_idx_i][min_idx_j])
